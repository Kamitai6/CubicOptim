...
point = [[0.2, 3.2, 0.0], [1.1, 3.15, 90.0], [1.25, 0.0, 0.0], [
    2.97, 0.33, 0.0], [2.25, 1.35, 0.0], [2.97, 2.4, 0.0], [3.1, 3.6, 0.0]]
estimate_number = [1, 1, 1, 1, 2, 2, 1]

vel_acc = [[0.0, 5.0], [2.0, 2.0], [-1.0, 5.0], [1.0, 3.0],
           [1.0, 3.0], [1.0, 3.0], [0.0, 3.0]]  # -1 --> stop
v_max = 3.0
frequency = 1000  # hz
...
def main():
    ...
    p_x, p_y, r_x, r_y, _, length, length_list = calcTrajectory(result.x)
    ...
    for i in range(len(vel_acc)):
        ...
        x_start += sum([
            length[i*2+e+j] for j in range(1, estimate_number[i]+1)
        ])
